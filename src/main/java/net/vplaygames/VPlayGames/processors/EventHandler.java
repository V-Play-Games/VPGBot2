...
public class EventHandler extends ListenerAdapter {
    ...
    public void process(GuildMessageReceivedEvent e) {
        if (Bot.closed && Bot.commands.get(e.getMessage().getContentRaw()) == Bot.activateCommand) {
            Bot.activateCommand.run(e);
        }
        consecutiveErrors = 0;
        currentTime = System.currentTimeMillis();
        if (!e.getAuthor().isBot() && e.getChannel().canTalk() && (OtherEvents.process(e) || Commands.process(e))) {
            log();
        }
    }
